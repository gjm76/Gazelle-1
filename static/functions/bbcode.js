...
function video(textID) {
    var linkAddr = prompt("Please enter the url ", "https://www.youtube.com/");

    if (linkAddr && linkAddr != "https://www.youtube.com/") insert('[video=' + linkAddr + ']', textID);
}

function flash(textID) {
    var linkAddr = prompt("Please enter the url for the flash object", "https://");
    if (linkAddr && linkAddr != "https://") {
...
function image_prompt(textID) {
    ...
    do {
        link = prompt("Please enter the full URL for your image", "https://");
        if (img_regex.test(link) == false && link != "https://" && link){
            alert("Not a valid image url");
        } else break;
    } while(true)
    if (link != "https://" && link) wrap('img', link, '', textID);
}
