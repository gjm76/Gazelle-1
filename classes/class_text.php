...
class text
{
    ...
    {
        include SERVER_ROOT.'/classes/smileys.php';
    ...
    public function proxify_url($url) {
        ...
        if (DEFINED('IMAGE_PROXY_URL') && !$this->validate_imageurl($url)) {
    ...
    public function local_url($url, $always_full = false) {
        ...
        $regex = INTERNAL_URLS_REGEX;
        if (preg_match($regex, $url)) {
            $prepared_url = '';
            if ($always_full) {
                $prepared_url .= ($SSL) ? 'https://' : 'http://';
                $prepared_url .= $_SERVER['HTTP_HOST'];
            }
        ...
        while ($i < $Len) {
            ...
            switch ($TagName) {
                ...
                case 'hide':
                    ...
                    break; // not seen
                case 'donate':
                    $Array[$ArrayPos] = array('Type' => 'donate', 'Val'=>$Block);
                    break;
                case 'spoiler':
                    $Array[$ArrayPos] = array('Type' => $TagName, 'Attr' => $Attrib, 'Val' => $this->parse($Block));
                    break;
...
//echo "</pre>";
            ...
            switch ($Block['Type']) {
                ...
                case 'video':
                    ...
                    if ($this->NoMedia > 0) {
                        ...
                        elseif ($YoutubeID != '')
                            $Str.='<iframe height="385" width="640" src="//www.youtube-nocookie.com/embed/' . $YoutubeID . '" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>';
                        elseif ($VimeoID != '')
                            $Str.='<iframe src="https://player.vimeo.com/video/' . $VimeoID . '" width="640" height="385" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>';
                        else
    ...
    function getQueryParameters($value, $section, $customRegex = [])
    {
        ...
        $Pattern = '/^(?<id>'.$idRegex.')$|^(' . SITE_URL . ')?\/'.$section.'\.php\?(?<queryString>.*)/i';
        $Matches = array();
    ...
    {
        $Matches = $this->getQueryParameters($value, 'articles', ['id' => '[a-z0-9\-\_.()\@&]+']);
        if (!empty($Matches['topic'])) {
            $Title = getArticleTitle($Matches['topic']);
            if ($Title) {
                $Str = '<a href="articles.php?topic='.$Matches['topic'].'"><span class="taglabel">Article: </span>'.$Title.'</a>';
            } else {
    ...
    {
        $Matches = $this->getQueryParameters($value, 'torrents', ['id' => '\d+', 'postid' => '\d+']);
        if (empty($Matches['id']) && !empty($Matches['torrentid'])) $Matches['id'] = $Matches['torrentid'];
        if (!empty($Matches['id'])) {
            $Groups = get_groups(array($Matches['id']), true, false);
            if (!empty($Groups['matches'][$Matches['id']])) {
                $Group = $Groups['matches'][$Matches['id']];
                $Str = '<a href="torrents.php?id='.$Matches['id'].'"><span class="taglabel">Torrent: </span>'.$Group['Name'].'</a>';
                if (!empty($Matches['postid'])) {
                    $Str .= '&nbsp;&nbsp;<a href="torrents.php?id='.$Matches['id'].'&postid='.$Matches['postid'].'#post'.$Matches['postid'].'"><span class="taglabel">comment: </span>'.$Matches['postid'].'</a>';
                }
            } else {
    ...
    {
        $Matches = $this->getQueryParameters($value, 'requests', ['id' => '\d+', 'page' => '\d+', 'postid' => '\d+']);
        if (!empty($Matches['id'])) {
            $Request = get_requests(array($Matches['id']), true);
            if (!empty($Request['matches'][$Matches['id']])) {
                $Request = $Request['matches'][$Matches['id']];
                $Str = '<a href="requests.php?action=view&id='.$Matches['id'].'"><span class="taglabel">Request: </span>'.$Request['Title'].'</a>';
                if (!empty($Matches['postid'])) {
    ...
    {
        $Matches = $this->getQueryParameters($value, 'collages', ['id' => '\d+', 'postid' => '\d+']);
        if (!empty($Matches['id'])) {
            $Title = getCollageName($Matches['id']);
            if ($Title) {
                $Str = '<a href="collages.php?id='.$Matches['id'].'"><span class="taglabel">Collage: </span>'.$Title.'</a>';
                if (!empty($Matches['postid'])) {
    ...
    function threadTag($value)
    {
        ...
        if (!empty($Matches['threadid'])) {
            ...
            if ($Title) {
                $Str = '<a href="forums.php?action=viewthread&threadid='.$Matches['threadid'].'"><span class="taglabel">Thread: </span>'.$Title.'</a>';
                if (!empty($Matches['postid'])) {
    ...
    function forumTag($value)
    {
        ...
        if (!empty($Matches['forumid'])) {
            ...
            if ($Title) {
                $Str = '<a href="forums.php?action=viewforum&forumid='.$Matches['forumid'].'"><span class="taglabel">Forum: </span>'.$Title.'</a>';
            } else {
    ...
    function userTag($value)
    {
        ...
        if (!empty($Matches['id'])) {
            ...
            if ($Title) {
                $Str = '<a href="user.php?id='.$Matches['id'].'"><span class="taglabel">User: </span>'.$Title.'</a>';
            } else {
    ...
    public function display_bbcode_assistant($textarea, $AllowAdvancedTags, $start_num_smilies = 0, $load_increment = 240, $load_increment_first = 30)
    {
            ...
            <tbody><tr>
                    <td class="colhead" style="padding: 2px 2px 0px 6px">
                        ...
                        <div class="bb_buttons_left">
                            ...
                            <a class="bb_button" onclick="image_prompt('<?= $textarea; ?>')" title="Image: [img]https://image_url[/img]" alt="Image">Img</a>
                         ...
                         <?php /*   <a class="bb_button" onclick="video('<?= $textarea; ?>')" title="Youtube video: [video=https://www.youtube.com/v/abcd]" alt="Youtube">Video</a>
        <?php if (check_perms('site_moderate_forums')) { ?>
                            <a class="bb_button" onclick="flash('<?= $textarea; ?>')" title="Flash object: [flash]https://url.swf[/flash]" alt="Flash">Flash</a>
        <?php       } ?>*/ ?>
                        ...
                        <div class="bb_buttons_right">
                            <img class="bb_icon" src="<?= get_symbol_url('align_center.svg') ?>" onclick="wrap('align','','center', '<?= $textarea; ?>')" title="Center Align text: [align=center]text[/align]" alt="Center" />
                            <img class="bb_icon" src="<?= get_symbol_url('align_left.svg') ?>" onclick="wrap('align','','left', '<?= $textarea; ?>')" title="Left Align text: [align=left]text[/align]" alt="Left" />
                            <img class="bb_icon" src="<?= get_symbol_url('align_justify.svg') ?>" onclick="wrap('align','','justify', '<?= $textarea; ?>')" title="Justify text: [align=justify]text[/align]" alt="Justify" />
                            <img class="bb_icon" src="<?= get_symbol_url('align_right.svg') ?>" onclick="wrap('align','','right', '<?= $textarea; ?>')" title="Right Align text: [align=right]text[/align]" alt="Right" />
                            <img class="bb_icon" src="<?= get_symbol_url('text_uppercase.svg') ?>" onclick="text('up', '<?= $textarea; ?>')" title="To Uppercase" alt="Up" />
                            <img class="bb_icon" src="<?= get_symbol_url('text_lowercase.svg') ?>" onclick="text('low', '<?= $textarea; ?>')" title="To Lowercase" alt="Low" />
                        </div>
        ...
        ?>
                            ...
                            <a class="bb_button" onclick="tag('mediainfo', '<?= $textarea; ?>')" title="Media Info display: [mediainfo] Media Info [/mediainfo]" alt="mediainfo">Media Info</a>
