...
<?php
            $GIT=explode(PHP_EOL, shell_exec("git log -1"));
...
<?php
        foreach ($Queries as $Key => $Query) {
            list($Params,$Time,$Result) = $Query;
?>
        <tr valign="top">
            <td class="debug_data debug_sphinx_data"><a href="#" onclick="$('#debug_sphinx_<?=$Key?>').toggle();return false;"><pre><?=str_replace("\t", '	', display_str($Params))?></pre></a></td>
            <td class="rowa" style="width:130px;" align="left"><?=number_format($Time, 5)?> ms</td>
        </tr>
        <tr id="debug_sphinx_<?=$Key?>" class="hidden">
            <td align="left" class="debug_data debug_sphinx_data">
                <pre><?=display_str(print_r($Result))?></pre>
            </td>
        </tr>
<?php
        }
