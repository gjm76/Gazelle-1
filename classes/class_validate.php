...
class validate
{
    ...
    public function SetFields($FieldName,$Required,$FieldType,$ErrorMessage,$Options=array())
    {
        ...
        if (isset($Options['prohibitgif'])) {
            $this->Fields[$FieldName]['ProhibitGIF']=$Options['prohibitgif'];
        }
    }
                      ...
                      if ($ValidateVar!="" || !empty($Field['Required']) || $Field['Type']=="date") {
                            if ($Field['Type']=="string") {
                                ...
                                // 3) optional filesize of image (probably should not use in large batches as has to fetch remote image)
                                    ...
                                    $ProhibitGIF = isset($Field['ProhibitGIF']) ? $Field['ProhibitGIF'] : false;

                                    // get validation result
                                    $result = validate_imageurl($ValidateVar, $MinLength, $MaxLength, $WLRegex, $ProhibitGIF);
                                    if ($result !== TRUE) return "$Field[ErrorMessage]<br/>$result";
                                ...
                                // 3) optional filesize of image (probably should not use in large batches as has to fetch remote image)
                                    ...
                                    // Get parameters to validate against from fields set
                                    if (isset($Field['MaxLength'])) { $MaxLength=$Field['MaxLength']; } else { $MaxLength=512; }
                                    if (isset($Field['MinLength'])) { $MinLength=$Field['MinLength']; } else { $MinLength=12; }
                                    if (isset($Field['MaxFilesize'])) { $MaxFileSize=$Field['MaxFilesize']; } else { $MaxFileSize=-1; }
                                    if (isset($Field['MaxWidth'])) { $MaxWidth=$Field['MaxWidth']; } else { $MaxWidth=-1; }
                                    if (isset($Field['MaxHeight'])) { $MaxHeight=$Field['MaxHeight']; } else { $MaxHeight=-1; }
                                    ...
                                    //strip [img] tags
                                    $screens = preg_replace('(\[img\])', '', $ValidateVar); 
                                    $screens = preg_replace('(\[\/img\])', '', $screens);
                                    
                                    // get validation result
                                    $result = validate_imageurl($screens, $MinLength, $MaxLength, $WLRegex);
                                    if ($result !== TRUE) return "$Field[ErrorMessage]<br/>$result";

                                    // check image dimensions if max dimensions are specified
                                    if ($MaxWidth>=0 && $MaxHeight>=0) {
                                        $image_attribs = getimagesize($screens);
                                        if ($image_attribs!==FALSE) { // i guess we should ignore it if it fails .. hmmm...
                                            list($width, $height, $type, $attr) = $image_attribs;
                                            if ($width>$MaxWidth || $height > $MaxHeight)
                                                return "$Field[ErrorMessage]<br/>Image dimensions are too big; width: {$width}px  height: {$height}px<br/>Max Image dimensions; width: {$MaxWidth}px  height: {$MaxHeight}px<br/>File: $screens";
                                        }
                                    }

                                    // check remote filesize if max specififed
                                    if ($MaxFileSize>=0) {
                                        $filesize = get_remote_file_size($screens);
                                        //if ($filesize<0) return "error getting filesize";
                                        if ($filesize>$MaxFileSize)
                                            return "$Field[ErrorMessage]<br/>Filesize is too big: " . get_size($filesize). "<br/>MaxFilesize: ".get_size($MaxFileSize). "<br/>File: $screens";
                                    }
                                    ...
                                    if ($num && $num >= $MinImages) { // if there are no img tags then it validates
                                        for ($j=0;$j<$num;$j++) {
                                            ...
                                            $result = validate_imageurl($imageurls[1][$j], 12, 255, $WLRegex, $ProhibitGIF);
                                        ...
                                        //if (!empty($Field['Required'])) {
                                            ...
                                            if ($MinImages == 1)
                                                $Field['ErrorMessage'] = "There are no images in your description. You are required to add $MinImages screenshots.";
                                            else
                                                $Field['ErrorMessage'] = "There are not enough images in your description. You are required to add $MinImages screenshots.";
