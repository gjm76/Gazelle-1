...
if (empty($_POST['class']) || !in_array($_POST['class'], array("<= ".$Classes[FREELANCER]['Level'], "= ".$Classes[RESEARCHER]['Level'], "<= ".$Classes[DESIGNER]['Level'], "<= ".$Classes[PRESENTER]['Level'], ">= ".$Classes[PUBLICIST]['Level'], ">= ".$Classes[EDITOR]['Level']))) {
    $REQUIRED_CLASS    = "<= ".$Classes[FREELANCER]['Level'];
} else {
...
if ($OtherID && ($Cost <= $Credits)) {
    ...
    Switch($Action){  // atm hardcoded in db:  givecredits, givegb, gb, slot, title, badge
        case 'givecredits':
            ...
            $Summary = sqltime().' | +'.ucfirst(number_format($Value)." credits | You received a special gift of ".number_format($Value)." credits from an anonymous member");
            $UpdateSetOther[]="i.BonusLog=CONCAT_WS( '\n', '$Summary', i.BonusLog)";
            $UpdateSetOther[]="m.Credits=(m.Credits+'$Value')";

            $Summary = sqltime().' | - '.ucfirst(number_format($Cost)." credits | You gave a special gift of ".number_format($Value)." credits to an anonymous member");
        ...
        case 'givegb':  // no test if user had download to remove as this could violate privacy settings
            ...
            $Summary = sqltime()." | ".ucfirst("you received a special gift of -$Value gb from an anonymous member");
            $UpdateSetOther[]="i.BonusLog=CONCAT_WS( '\n', '$Summary', i.BonusLog)";

            $Summary = sqltime()." | -$Cost credits | ".ucfirst("you gave a special gift of -$Value gb to an anonymous member");
            $UpdateSet[]="i.BonusLog=CONCAT_WS( '\n', '$Summary', i.BonusLog)";
            $UpdateSet[]="m.Credits=(m.Credits-'$Cost')";
