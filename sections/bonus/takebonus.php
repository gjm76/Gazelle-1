    ...
    if ($OtherID && ($Cost <= $Credits)) {
        ...
        Switch($Action){  // atm hardcoded in db:  givecredits, givegb, gb, slot, title, badge
            case 'badge' :
                ...
                $Summary = sqltime().' - '.ucfirst("user bought $Title badge. Cost: $Cost cubits");
                $UpdateSet[]="i.AdminComment=CONCAT_WS( '\n', '$Summary', i.AdminComment)";
                $Summary = sqltime()." | -$Cost cubits | ".ucfirst("you bought a $Title badge.");
            ...
            case 'givecredits':
                ...
                $Summary = sqltime().' - '.ucfirst("user gave a gift of ".$Value." cubits to {$P['othername']} Cost: $Cost cubits");
                $UpdateSet[]="i.AdminComment=CONCAT_WS( '\n', '$Summary', i.AdminComment)";

                $Summary = sqltime().' - '.ucfirst("user received a gift of ".$Value." cubits from {$LoggedUser['Username']}");
                $UpdateSetOther[]="i.AdminComment=CONCAT_WS( '\n', '$Summary', i.AdminComment)";

                $Summary = sqltime()." | +".$Value." cubits | ".ucfirst("you received a gift of ".$Value." cubits from {$LoggedUser['Username']}");
                $UpdateSetOther[]="i.BonusLog=CONCAT_WS( '\n', '$Summary', i.BonusLog)";
                $UpdateSetOther[]="m.Credits=(m.Credits+'$Value')";
                $Summary = sqltime()." | -".$Cost." cubits | ".ucfirst("you gave a gift of ".$Value." cubits to {$P['othername']}");
                $UpdateSet[]="i.BonusLog=CONCAT_WS( '\n', '$Summary', i.BonusLog)";
                $UpdateSet[]="m.Credits=(m.Credits-'$Cost')";
                $ResultMessage=$Summary;

                send_pm($OtherID, 0, "Black Market - You received a gift of Cubits",
                            "[br]You received a gift of ".$Value." cubits from [user={$LoggedUser['Username']}]{$LoggedUser['Username']}[/user]");
            ...
            case 'gb':
                ...
                if ($LoggedUser['BytesDownloaded'] <= 0) {
                    ...
                    $Summary = sqltime().' - '.ucfirst("user bought -$Value gb. Cost: $Cost cubits");
                    if($LoggedUser['BytesDownloaded'] < $ValueBytes)
                        $Summary .= " | NOTE: Could only remove ". get_size($LoggedUser['BytesDownloaded']);
                    $UpdateSet[]="i.AdminComment=CONCAT_WS( '\n', '$Summary', i.AdminComment)";

                    $Summary = sqltime()." | -$Cost cubits | ".ucfirst("you bought -$Value gb.");
                    if($LoggedUser['BytesDownloaded'] < $ValueBytes)
                        $Summary .= " | NOTE: Could only remove ". get_size($LoggedUser['BytesDownloaded']);
                    $UpdateSet[]="i.BonusLog=CONCAT_WS( '\n', '$Summary', i.BonusLog)";
            ...
            case 'givegb':  // no test if user had download to remove as this could violate privacy settings
                $Summary = sqltime().' - '.ucfirst("user gave a gift of -$Value gb to {$P['othername']} Cost: $Cost cubits");
                $UpdateSet[]="i.AdminComment=CONCAT_WS( '\n', '$Summary', i.AdminComment)";
                ...
                $UpdateSetOther[]="i.BonusLog=CONCAT_WS( '\n', '$Summary', i.BonusLog)";
                $Summary = sqltime()." | -$Cost cubits | ".ucfirst("you gave a gift of -$Value gb to {$P['othername']}.");
                $UpdateSet[]="i.BonusLog=CONCAT_WS( '\n', '$Summary', i.BonusLog)";
                $UpdateSet[]="m.Credits=(m.Credits-'$Cost')";

                send_pm($OtherID, 0, db_string("Black Market - You received a gift of -gb"),
            ...
            case 'slot':
                ...
                $Summary = sqltime().' - '.ucfirst("user bought $Value slot".($Value>1?'s':'').". Cost: $Cost cubits");
                $UpdateSet[]="i.AdminComment=CONCAT_WS( '\n', '$Summary', i.AdminComment)";
                $Summary = sqltime()." | -$Cost cubits | ".ucfirst("you bought $Value slot".($Value>1?'s':'').".");
                $UpdateSet[]="i.BonusLog=CONCAT_WS( '\n', '$Summary', i.BonusLog)";
            ...
            case 'invite':
                ...
                if($LoggedUser['HnR']) error("You can't buy an invite while having HnRs");
                if($LoggedUser['Class']<$SiteOptions['InviteBuyThreshold']) error("You class is below the minimum allowed.");

                $Summary = sqltime().' - '.ucfirst("user bought $Value invite".($Value>1?'s':'').". Cost: $Cost cubits");
                $UpdateSet[]="i.AdminComment=CONCAT_WS( '\n', '$Summary', i.AdminComment)";
                $Summary = sqltime()." | -$Cost cubits | ".ucfirst("you bought $Value invite".($Value>1?'s':'').".");
                $UpdateSet[]="i.BonusLog=CONCAT_WS( '\n', '$Summary', i.BonusLog)";
                $UpdateSet[]="m.Invites=(m.Invites+'$Value')";
                $UpdateSet[]="m.Credits=(m.Credits-'$Cost')";
                $ResultMessage=$Summary;
                write_log("Invite was bought by " .$LoggedUser['Username']);
                break;

            case 'pfl':
                $Summary = sqltime().' - '.ucfirst("user bought $Value hour".($Value>1?'s':'')." personal freeleech. Cost: $Cost cubits");
                $UpdateSet[]="i.AdminComment=CONCAT_WS( '\n', '$Summary', i.AdminComment)";
                $Summary = sqltime()." | -$Cost cubits | ".ucfirst("you bought $Value hour".($Value>1?'s':'')." of personal freeleech.");
                $UpdateSet[]="i.BonusLog=CONCAT_WS( '\n', '$Summary', i.BonusLog)";
                $UpdateSet[]="m.Credits=(m.Credits-'$Cost')";
            ...
            case 'title':
                ...
                if (!$NewTitle) {
                    ...
                    if ($tlen > 32) {
                        ...
                        $Summary = sqltime().' - '.ucfirst("user bought a new custom title ''$NewTitle''. Cost: $Cost cubits");
                        $UpdateSet[]="i.AdminComment=CONCAT_WS( '\n', '$Summary', i.AdminComment)";
                        $Summary = sqltime()." | -$Cost cubits | ".ucfirst("you bought a new custom title ''$NewTitle''.");
                        $UpdateSet[]="i.BonusLog=CONCAT_WS( '\n', '$Summary', i.BonusLog)";
                ...
                if (!$GroupID) {
                    ...
                    if ($DB->record_count()==0)
                        ...
                        if ($OwnerID != $LoggedUser['ID']) {
                            ...
                            // make torrent FL
                            freedouble_groups($GroupID, 1, 0, true);

                            $Summary = sqltime().' - '.ucfirst("user bought universal freeleech ($Value gb+) for torrent [torrent]{$GroupID}[/torrent]. Cost: $Cost cubits");
                            $UpdateSet[]="i.AdminComment=CONCAT_WS( '\n', '$Summary', i.AdminComment)";
                            $Summary = sqltime()." | -$Cost cubits | ".ucfirst("you bought a universal freeleech ($Value gb+) for torrent [torrent]{$GroupID}[/torrent]");
                            $UpdateSet[]="i.BonusLog=CONCAT_WS( '\n', '$Summary', i.BonusLog)";
                            $UpdateSet[]="m.Credits=(m.Credits-'$Cost')";
                            $ResultMessage= sqltime()." | -$Cost cubits | ".ucfirst("you bought a universal freeleech ($Value gb+) for torrent $TName"); ;
                        }
                    }
                }
                break;
