            ...
            if ($Attempts>5) { // Only 6 allowed login attempts, ban user's IP
                ...
                if ($Bans>9) { // Automated bruteforce prevention
                    ...
                    if ($DB->record_count() > 0) {
                        ...
                        if ($Reason != "Automated ban per >2 failed login attempts") {
    ...
    if (isset($_POST['username']) && !empty($_POST['username']) && isset($_POST['password']) && !empty($_POST['password'])) {
        ...
        if (!$Err) {
            ...
            $DB->query("SELECT
                ...
                AND Username<>''");
            list($UserID,$Username,$PermissionID,$CustomPermissions,$PassHash,$TwoFactor,$Secret,$OldPassHash,$Enabled)=$DB->next_record(MYSQLI_NUM, array(2));
            if (strtotime($BannedUntil)<time()) {
                            //die("{$_POST['password']}, PassHash: $PassHash, Secret: $Secret, Generated PassHash:".make_hash($_POST['password'],$Secret));
                if (!empty($PassHash)){
                    $CheckHash=make_hash($_POST['password'],$Secret);
                } else {
                    $PassHash = $OldPassHash;
                    $CheckHash=make_old_hash($_POST['password']);
                    if($PassHash == $CheckHash) {
                        ...
                        $DB->query("UPDATE
                            ...
                            m.Secret='".db_string($Secret)."',
                            m.OldPassHash=NULL,
                            i.AuthKey='".db_string($AuthKey)."',
                            i.RunHour=FLOOR( RAND() * 24 ),
                            i.AdminComment=CONCAT( '".sqltime()." - User password synced to new method.\n', i.AdminComment)
                            WHERE m.ID='".db_string($UserID)."'
                            AND i.UserID=m.ID");
                    }
                }

                if ($UserID && $PassHash==$CheckHash) {
