...
if (!isset($_REQUEST['authkey']) || !isset($_REQUEST['torrent_pass'])) {
    ...
    $SSLTracker  = $LoggedUser['SSLTracker'];
} else {
    ...
    list($UserID,$DownloadAlt,$SSLTracker)=array_shift($UserInfo);
    if (!$UserID) { error(403); }
...
list($GroupID,$Name, $Size, $FreeTorrent, $DoubleSeed, $InfoHash, $File_Name) = array_shift($Info); // used for generating the filename
...
if ($SSLTracker) {
    $Tor->set_announce_url(SSL_ANNOUNCE_URL.'/'.$TorrentPass.'/announce');
} else {
...
//$TorrentName = $Name;
$TorrentName = $File_Name;

if (!$TorrentName) { $TorrentName="No Name"; }

$TorrentName = "[".SITE_NAME."].".$TorrentName.".".$GroupID;
