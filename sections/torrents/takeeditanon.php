...
if (!$CanEdit) {
    ...
    $CanEdit = check_perms('site_upload_anon') && $AuthorID == $LoggedUser['ID'] && time_ago($$TorrentTime)< TORRENT_EDIT_TIME && $Review['Status'] != 'Okay' ||
     check_perms('site_upload_anon') && $AuthorID == $LoggedUser['ID'] && $Review['Status'] != 'Okay' && $Review['Status'] ||    
     check_perms('site_edit_override_timelock');
}
