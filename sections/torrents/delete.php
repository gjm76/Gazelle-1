...
$Review = get_last_review($TorrentID);
...
if (!$CanEdit) {
	if ($LoggedUser['ID'] == $UserID) {
		if (time_ago($Time)< TORRENT_EDIT_TIME && $Review['Status'] != 'Okay' || $Review['Status'] != 'Okay' && $Review['Status']) {
       $CanEdit = true;
		}
   }
}
...
$DB->query("SELECT Episode FROM torrents WHERE ID = $TorrentID");
list($Episode) = $DB->next_record();
if (!($Episode > 0)) {
    $ReasonOnchange = ' onchange="WarnSeason()"';
}
        ...
        <div class="box pad" >
            <div class="pad">
                <form action="torrents.php" method="post">
                    ...
                    <select name="reason" id="reason"<?=$ReasonOnchange?>>
...
?>
<div id="all_reports" style="width: 96%; margin:0px auto;">
<?php
