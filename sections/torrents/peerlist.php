...
<table>
    ...
    while (list($PeerUserID, $Size, $Username, $Active, $Connectable, $Uploaded, $Remaining, $UserAgent,
        ...
        <tr>
            ...
            <td title="<?= get_size($UpSpeed, 2)?>/s"><?= get_size($Uploaded) ?></td>
            <td title="<?= get_size($DownSpeed, 2)?>/s"><?= get_size($Size - $Remaining, 2) ?></td>
            <td title="Ratio: <?= number_format($Uploaded / ($Size - $Remaining), 3) ?>"><?= number_format(($Size - $Remaining) / $Size * 100, 2) ?></td>
            <td title="Time: <?= time_span($Timespent) ?>"><?= display_str($UserAgent) ?></td>
        </tr>
        <?php
    }
    ?>
