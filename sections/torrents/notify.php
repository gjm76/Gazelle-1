...
?>
    ...
    <form id="notificationform_<?=$ID?>">
          <tr class="colhead">
                ...
                <td class="sign"><a href="<?=header_link('Snatches') ?>"><img src="static/styles/themes/<?=$LoggedUser['StyleName']?>/images/snatched.svg" alt="↺" title="Snatches" /></a></td>
                <td class="sign"><a href="<?=header_link('Seeders') ?>"><img src="static/styles/themes/<?=$LoggedUser['StyleName']?>/images/seeders.svg" alt="∧" title="Seeders" /></a></td>
                <td class="sign"><a href="<?=header_link('Leechers') ?>"><img src="static/styles/themes/<?=$LoggedUser['StyleName']?>/images/leechers.svg" alt="∨" title="Leechers" /></a></td>
          </tr>
<?php
        ...
        foreach ($FilterResults as $Result) {
            list($TorrentID, $GroupID, $Image, $GroupName, $GroupCategoryID, $TorrentTags, $Size, $FileCount,
                $Snatched, $Seeders,
                $Leechers, $NotificationTime, $FreeTorrent, $DoubleTorrent, $IsAnon, $FilePath, $Username, $LogInDB, $UnRead, $FilterLabel, $FilterLabel,
                $TVMAZE) = $Result;
            ...
            $Icons = torrent_icons(array('FreeTorrent'=>$FreeTorrent,'DoubleTorrent'=>$DoubleTorrent), $TorrentID, $Review, in_array($GroupID, $Bookmarks));
...
?>
          <tr class="torrent <?=($IsMarkedForDeletion?'redbar':"row$row")?>" id="torrent<?=$TorrentID?>">
                ...
                <td class="center cats_cols">
                 <div title="<?php echo empty($TVMAZE) ? $NewCategories[$GroupCategoryID]['tag'] : 'Go to show'; ?>">
<?php             if($TVMAZE) { ?>
                   <a href="torrents.php?action=show&showid=<?=$TVMAZE?>">
<?php             } ?>                  
                    <img src="<?='static/common/caticons/'.$NewCategories[$GroupCategoryID]['image']?>" />
<?php             if($TVMAZE) { ?>
                   </a> 
<?php             } ?>
                 </div>
...
<?php               $TorrentUsername = anon_username($Username, $IsAnon);
                    if (!$LoggedUser['HideFloat']) {
                        $Overlay = get_overlay_html($GroupName, $TorrentUsername, $Image, $Seeders, $Leechers, $Size, $Snatched, $FilePath); ?>
                        <script>
