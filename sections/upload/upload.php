...
foreach ($Whitelist as $ImageHost) {
    ...
    if (!empty($Link) && $Text->valid_url($Link)) {
        ?><a href="<?= $Link ?>"  target="_blank"><img src="<?=STATIC_SERVER?>common/symbols/offsite.svg" width="16" height="16" style="" alt="Goto <?= $Host ?>" /></a>
        ...
        <div class="box pad shadow">
            <form action="" class="center" enctype="multipart/form-data"  method="post" onsubmit="return ($('#template').raw().value!=0);">
                <div style="margin:5px auto 10px;" class="nobr center">
                    <label for="template"><span style="color:red"> >> You must use an upload template: << </span></label>
...
/* -------  Retrieve banner  ------- */
if($TorrentForm->Torrent['TVMAZE']) {
  $ID = $TorrentForm->Torrent['TVMAZE'];
  $DB->query("SELECT
  BannerLink
  FROM torrents_banners
  WHERE TVMazeID = $ID");

  list($TorrentForm->Torrent['Image']) = $DB->next_record();
}
