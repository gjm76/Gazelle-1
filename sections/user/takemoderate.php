...
$ResetHnRWatch = (isset($_POST['ResetHnRWatch']))? 1 : 0;
...
if ($_POST['ResetHnRWatch'] && check_perms('users_edit_reset_keys')) {
    $DB->query("UPDATE users_info SET HnRWatchEnds='0000-00-00 00:00:00', HnRWatchTimes='0' WHERE UserID='$UserID'");
    $EditSummary[]='HnR Watch history reset';
}
...
if ($_POST['ResetIPHistory'] && check_perms('users_edit_reset_keys')) {
    ...
    $DB->query("UPDATE users_main SET ipcc= '' WHERE ID='$UserID'"); // reset country flag as well
...
if ($_POST['ResetHnRs'] && check_perms('users_edit_reset_keys')) {
    $DB->query("UPDATE xbt_snatched SET seedtime = '" . ($SiteOptions['HnRSeason'] + 1) . "' WHERE uid='$UserID' AND seedtime < '" . $SiteOptions['HnRSeason'] . "'");
    $DB->query("UPDATE users_main SET HnR = 0 WHERE ID='$UserID'");
    $EditSummary[]='HnRs cleared';
    $Cache->delete_value('users_torrents_snatched_' . $UserID);
    $Cache->delete_value('user_info_'.$UserID);
...
if ($BonusCredits!=$Cur['Credits'] && ((check_perms('users_edit_credits') && $UserID != $LoggedUser['ID'])
        ...
        if ($Creditschange>=0) $Creditschange = "+".$Creditschange;
        else $Creditschange = $Creditschange;
...
if ($DisableLeech!=$Cur['can_leech'] && check_perms('users_disable_any')) {
    ...
    // Only send leech status to tracker if user is currently enabled!
    if ($EnableUser == '1')
...
if ($PersonalFreeLeech != 1 && ($PersonalFreeLeech > 1 || ($PersonalFreeLeech == 0 && $Cur['personal_freeleech'] > sqltime())) &&
   ((check_perms('users_edit_pfl') && $UserID != $LoggedUser['ID']) || (check_perms('users_edit_own_pfl') && $UserID == $LoggedUser['ID']))) {
    ...
    // Only send PFL to tracker if user is currently enabled!
    if ($EnableUser == '1')
...
if ($ResetPasskey == 1 && check_perms('users_edit_reset_keys')) {
    ...
    // Only send new passkey to tracker if user is currently enabled!
    if ($EnableUser == '1')
