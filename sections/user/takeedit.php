...
$Val->SetFields('avatar',0,'image', 'Avatar: The image URL you entered was not valid.',
                 array('regex' => $whitelistregex, 'prohibitgif' => true, 'maxlength' => 255, 'minlength' => 6, 'maxfilesizeKB'=>-1, 'dimensions'=>array($Permissions['MaxAvatarWidth'],$Permissions['MaxAvatarHeight'])));
$Val->SetFields('info',0,'desc','Info',array('regex'=>$whitelistregex,'minlength'=>0,'maxlength'=>20000));
$Val->SetFields('signature',0,'desc','Signature',array('regex'=>$whitelistregex,'prohibitgif' => true,'minlength'=>0,'maxlength'=>$Permissions['MaxSigLength'], 'dimensions'=>array(SIG_MAX_WIDTH, SIG_MAX_HEIGHT)));
$Val->SetFields('torrentsignature',0,'desc','Signature',array('regex'=>$whitelistregex,'prohibitgif' => true,'minlength'=>0,'maxlength'=>$Permissions['MaxSigLength']));
$Val->SetFields('email',1,"email","You did not enter a valid email address.");
$Val->SetFields('twofa', 0, "regex", "You did not enter a valid 2FA verification code.", array('regex' => '/^(([0-9]{6})|(REMOVE))$/'));
...
$Paranoia = array();
$Checkboxes = array('downloaded', 'uploaded', 'ratio', 'lastseen', 'requiredratio', 'seedsize', 'invitedcount');
...
if (isset($_POST['p_uploadanon'])) {
    $Paranoia[] = 'uploadanon';
}

$SimpleSelects = array('torrentcomments', 'collages', 'collagecontribs', 'uploads', 'seeding', 'leeching', 'snatched', 'grabbed', 'rated', 'follow', 'tags');
...
if (!empty($_POST['twofa'])) {
    $DB->query("
      SELECT TwoFactor, PassHash, Secret
      FROM users_main
      WHERE ID = $UserID");
    list($TwoFactor, $PassHash, $Secret) = $DB->next_record();
    if ($PassHash!=make_hash($_POST['cur_pass'],$Secret)) {
        $Err = "You did not enter the correct password.";
    }
    if (!$Err) {
        if (empty($TwoFactor)) {
            ...
            if ($TwoFA->verifyCode($_POST['twofasecret'], $_POST['twofa'])) {
                ...
                $Err = 'Invalid 2FA verification code.';
            }
        } else {
	    if ($_POST['twofa']==='REMOVE') {
            	$DB->query("UPDATE users_main SET TwoFactor=NULL WHERE ID = $UserID");
            } else $Err = 'Invalid input. Enter "REMOVE" in all-caps to disable 2FA';
        }
    }
}
//End 2FA
...
$Options['DisableLatestShows'] = (!empty($_POST['disablelatestshows']) ? 1 : 0);
...
$Options['UseTooltipster'] = (!empty($_POST['usetooltipster']) ? 0 : 1);
...
$Options['UseAnonymizer']   = (!empty($_POST['use_anonymizer']) ? 1 : 0);
