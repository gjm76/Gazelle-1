        ...
        case 'reply':
            ...
            if (!isset($_POST['requestid']) || !is_number($_POST['requestid'])) {
                error(0);
                  ...
                  if (empty(trim($_POST['body'])) && $_POST['body'] !== '0') {
            ...
            $Title = $_POST['title'];
            if($RequestorID && $RequestorID != $LoggedUser['ID']) {
               send_pm($RequestorID, 0, db_string('Comment received on your Request by '.$LoggedUser['Username'].''), 
                    db_string("You have received a comment from [url=/user.php?id=".$LoggedUser['ID']."] ".$LoggedUser['Username']."[/url] on your request [url=/requests.php?action=view&id=".$RequestID."&page=$Pages#post".$PostID."]".$Title."[/url]"));
            }
