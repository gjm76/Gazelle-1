...
list($RequestID, $RequestorID, $RequestorName, $TimeAdded, $LastVote, $CategoryID, $Title, $Image, $Description,
     $FillerID, $FillerName, $TorrentID, $TimeFilled, $GroupID, $UploaderID, $UploaderName, $TVMazeID) = $Request;
...
show_header('View request: '.$FullName, 'comments,requests,bbcode,jquery.cookie','details');

?>
<div class="thin">
    <h2>
<?php if (has_bookmarked('request', $RequestID)) { ?>
    <img class="icon" src="static/styles/themes/<?=$LoggedUser['StyleName']?>/images/bookmarked.svg" alt="bookmarked" title="You have this request bookmarked" />&nbsp;
<?php } ?>     
    <a href="requests.php">Requests</a> &gt; <?=$CategoryName?> &gt; <?=$DisplayLink?></h2>
    <a id="messages" ></a>
    <div class="linkbox">
<?php   if ($CanEdit) { ?>
...
<?php   }
    if (has_bookmarked('request', $RequestID)) { ?>
        <a href="#" class="__bookmark-request" data-requestid="<?=$RequestID?>" data-bookmarked>[Remove bookmark]</a>
<?php 	} else { ?>
        <a href="#" class="__bookmark-request" data-requestid="<?=$RequestID?>" >[Bookmark]</a>
<?php 	} ?>
...
<?php       if($TVMazeID) { ?>                          
           <a href="/torrents.php?action=show&showid=<?=$TVMazeID?>">
<?php       } else { ?>
           <a href="<?=get_search($SearchTitle)?>">
<?php       } ?> 
            <img class="details_banner" style="max-width: 220px;" src="<?=$Image?>" title="Go to <?=$SearchTitle?>" />
           </a>
        </div><br/>
<?php  } ?>
...
?>
      ...
      <div class="middle_column">
        ...
        <table>
            <?php /* <tr> I'm not sure it is necessary to display the title TWICE and with a stupid icon. I hate icons!
                <td class="label" style="text-align:center;">
                    <img src="<?=( 'static/common/caticons/' . $NewCategories[$CategoryID]['image'])?>" title="TVMaze ID: <?=$TVMazeID?>" />
                </td>
            ...
            <tr id="bounty">
                ...
                <td id="formatted_bounty" style="font-size: 1.8em;"><?=$RequestVotes['TotalBounty']?> Cubits</td>
            </tr>
            <tr id="fillerbounty">
                <td class="label">Filler's Bounty</td>
                <td id="formatted_bounty" style="font-size: 1.4em;"><?=$RequestVotes['TotalBounty']/2?> Cubits</td>
            </tr>
            ...
            <tr>
                <td class="label"></td>
                <td title="If you fill this request with another's torrent you will receive half of the bounty (<?=$RequestVotes['TotalBounty']/2?>) Cubits and the uploader will recieve the other half.">If you fill this request with your own torrent you will receive the full bounty of <strong><?=$RequestVotes['TotalBounty']?> Cubits</strong></td>
            </tr>
            <tr>
...
<?php 	if ($CanVote) { ?>
            <tr id="voting">
                ...
                <td>
                    <input type="text" id="amount_box" size="8" onchange="Calculate();" /> Cubits
            ...
            <tr>
                ...
                <td>
                    <form action="requests.php" method="get" id="request_form">
                        ...
                        If you add the entered <strong><span id="new_bounty">0 Cubits</span></strong> of bounty, your new stats will be: <br/>
                ...
                <td>
                    <form action="" method="post">
                        <div>
                            ...
                            <br/><input type="text" size="45" name="link" <?=(!empty($Link) ? "value='$Link' " : '')?>/>
                            <br/>Should be the permalink (PL) to the torrent
                            <br/>e.g. https://<?=NONSSL_SITE_URL?>/torrents.php?id=xxxx
                            <br/><br/>
                            <?php  if (check_perms('site_moderate_requests')) { ?>
                            <span title="Fill this request on behalf of user:">
                            Fill for user: <input type="text" size="45" name="user" title="the username of the user you are filling this for (they will be recorded as filling this request)" <?=(!empty($FillerUsername) ? "value='$FillerUsername' " : '')?>/>
                            </span><br/><br/>
                            <?php  } ?>
