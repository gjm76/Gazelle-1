...
show_header($Title, 'requests,jquery.cookie');

?>
<div class="thin">
    <h2><?php if ($BookmarkView) { ?><img src="static/styles/themes/<?= $LoggedUser['StyleName'] ?>/images/bookmarked.svg" alt="star" title="Bookmarked requests" /><?php } ?> &nbsp;Requests</h2>

    <div class="linkbox">
<?php 	if (!$BookmarkView) { ?>
        <a href="requests.php">[Search requests]</a>
<?php 		if (check_perms('site_submit_requests') && !$LoggedUser['HnR']) { ?>
...
<?php 		}
        if (!check_perms('site_submit_requests')) { ?>
            <br/><em> <a href="articles.php?topic=requests">You must be a Nugget with a ratio of at least 1.00 to be able to make a Request.</a></em>
<?php       }
    } else { ?>
...
<?php 	} ?>
            <table cellpadding="6" cellspacing="1" border="0" class="" width="100%">
                ...
                <tr>
                    ...
                    <td>
                        <input type="checkbox" name="show_filled" <?php  if (!empty($_GET['show_filled']) || (!$Submitted && !empty($_GET['type']) && $_GET['type'] == "filled")) { ?>checked="checked"<?php  } ?> />
                    </td>
                </tr>
...
?>
        <tr class="row<?=$Row?>">
            <?php /* <td class="center "> Hiding cat icons
...
<?php   		} ?>
            ...
            <td class="nobr">
                <span id="bounty_<?=$RequestID?>"><?=$RequestVotes['TotalBounty']?> Cubits</span>
            </td>
            <td>
<?php    		if ($IsFilled) { ?>
