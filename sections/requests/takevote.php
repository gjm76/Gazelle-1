...
if (intval($LoggedUser['TotalCredits']) < $Amount) {
    ...
    $DB->query("UPDATE requests SET LastVote = '".sqltime()."' WHERE ID = ".$RequestID);

    $Cache->delete_value('request_'.$RequestID);
    $Cache->delete_value('request_votes_'.$RequestID);

    // Subtract amount from user
    $DB->query("UPDATE users_main SET Credits = (Credits - ".$Amount.") WHERE ID = ".$LoggedUser['ID']);
    $DB->query("SELECT Title FROM requests WHERE ID = ".$RequestID);
    list($Title)=$DB->next_record();
    write_user_log($LoggedUser['ID'], "Removed -". $Amount. " Cubits for vote on request [url=/requests.php?action=view&id={$RequestID}]{$Title}[/url]");

    update_bonus_log($LoggedUser['ID'], sqltime()." | -".$Amount." credits | ".ucfirst("removed -". $Amount. " for vote on request [url=/requests.php?action=view&id={$RequestID}]{$Title}[/url]"));
