...
function get_user_languages($UserID)
{
    ...
    if ($Userlangs) {
        ...
        foreach ($Userlangs as $langresult) {
            if ($langresult['cc'])
                $Str .= '<img style="vertical-align: bottom" alt="['.$langresult['code'].']" title="['.$langresult['code'].'] '.$langresult['language'].'" src="https://'. SITE_URL.'/static/common/flags/iso16/'.$langresult['cc'].'.png" />&nbsp;';
            else
