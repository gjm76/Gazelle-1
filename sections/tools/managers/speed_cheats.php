...
$BanSpeed = isset($_GET['banspeed'])?(int) $_GET['banspeed']:$SiteOptions[KeepSpeed];
...
?>
                    <tr class="row<?=$row?>">
                        <td>
                           <a href="?action=speed_records&viewspeed=0<?=$viewlink?>&userid=<?=$UserID?>" title="View records for just <?=$Username?>"><img src="static/common/symbols/view.svg" alt="view" /></a>
                           <div style="display:inline-block">
<?php                          if (!$OnWatchlist) {
?>                            <a onclick="watchlist_add('<?=$UserID?>',true);return false;" href="#" title="Add <?=$Username?> to watchlist"><img src="static/common/symbols/watchedred.svg" alt="wl add" /></a><br/><?php
                           }
 ?>                           <a onclick="excludelist_add('<?=$UserID?>',true);return false;" href="#" title="Add <?=$Username?> to exclude list"><img src="static/common/symbols/watchedgreen.svg" alt="excl add" /></a><?php
                            //}
 ?>                        </div>
                           <a onclick="remove_records('<?=$UserID?>');return false;" href="#" title="Remove all speed records belonging to <?=$Username?> from stored records"><img src="static/common/symbols/trash.svg" alt="del records" /></a>
<?php
                            if ($Enabled=='1') {
                                if ($bantype==1) { ?>
                                <a href="tools.php?action=ban_speed_cheat<?=$pattern?>&banuser=1&returnto=cheats&userid=<?=$UserID?>" title="ban this slag for being a scummy cheater (speeding)"><img src="static/common/symbols/ban.svg" alt="ban" /></a>
<?php                               } else {?>
                                <a href="tools.php?action=ban_pattern_cheat<?=$pattern?>&banuser=1&returnto=cheats&userid=<?=$UserID?>" title="ban this slag for being a scummy cheater (pattern matching)"><img src="static/common/symbols/ban2.svg" alt="ban" /></a>
<?php                               }
                            }
                           ?>
                        </td>
...
?>
                        ...
                        <td class="center"><?=speed_span($MaxUpSpeed, $SiteOptions[KeepSpeed], 'red', get_size($MaxUpSpeed).'/s')?></td>
