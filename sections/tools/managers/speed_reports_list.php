...
$ViewSpeed = isset($_GET['viewspeed'])?(int) $_GET['viewspeed']:$SiteOptions[KeepSpeed];
...
<div class="thin">
    ...
    $TWatchlist = $DB->to_array('TorrentID');
            ...
            ?>
    <div class="head">Torrent watch list &nbsp;<img src="static/common/symbols/watched.svg" alt="view" /><span style="float:right;"><a href="#" onclick="$('#twatchlist').toggle();this.innerHTML=this.innerHTML=='(hide)'?'(view)':'(hide)';">(view)</a></span>&nbsp;</div>
...
<?php  if ($CanManage) { ?>
                            <select id="delrecordmins" name="delrecordmins" title="Delete unwatched records after this time">
                                <option value="0"<?=($SiteOptions[DeleteRecordsMins]==0?' selected="selected"':'');?>>&nbsp;asap&nbsp;&nbsp;</option>
<?php                               for ($i=1;$i<5;$i++) {
                                    $mins = $i * 15;  ?>
                                    <option value="<?=$mins?>" <?=($SiteOptions[DeleteRecordsMins]==$mins?' selected="selected"':'');?>>&nbsp;<?=time_span($mins*60);?>&nbsp;&nbsp;</option>
<?php                               }
                                for ($i=1;$i<25;$i++) {
                                    $mins = $i * 120;  ?>
                                    <option value="<?=$mins?>" <?=($SiteOptions[DeleteRecordsMins]==$mins?' selected="selected"':'');?>>&nbsp;<?=time_span($mins*60);?>&nbsp;&nbsp;</option>
...
<?php  } else { ?>
                            <input name="delrecords" type="text" style="width:130px;color:black;" disabled="disabled" value="<?=time_span($SiteOptions[DeleteRecordsMins]*60)?>" title="Delete unwatched records after this time" />
<?php  }  ?>
                </td>
                <td  class="center">
...
<?php  if ($CanManage) { ?>
                            <select id="keepspeed" name="keepspeed" title="Keep unwatched records over this speed">
                                <option value="524288"<?=($SiteOptions[KeepSpeed]==524288?' selected="selected"':'');?>>&nbsp;<?=get_size(524288);?>/s&nbsp;&nbsp;</option>
<?php                               for ($i=1;$i<21;$i++) {
                                    $speed = $i * 1048576;  ?>
                                    <option value="<?=$speed?>" <?=($SiteOptions[KeepSpeed]==$speed?' selected="selected"':'');?>>&nbsp;<?=get_size($speed);?>/s&nbsp;&nbsp;</option>
<?php                               } ?>
                            </select>
<?php  } else { ?>
                            <input type="text" name="keepspeed" style="width:130px;color:black;" disabled="disabled" value="<?=get_size($SiteOptions[KeepSpeed]);?>/s" title="Keep unwatched records over this speed" />
<?php  }  ?>
                </td>
<?php  if ($CanManage) { ?>
                <td  class="center">
...
<?php                           if ($_GET['userid']!=$UserID) {    ?>
                            <a href="?action=speed_records&viewspeed=0&userid=<?=$UserID?>" title="View records for just <?=$Username?>"><img src="static/common/symbols/view.svg" alt="view" /></a>
<?php                           }   ?>
                            <div style="display:inline-block">
<?php                           if (!$OnWatchlist) {
?>                           <a onclick="watchlist_add('<?=$UserID?>',true);return false;" href="#" title="Add <?=$Username?> to watchlist"><img src="static/common/symbols/watchedred.svg" alt="view" /></a><br/><?php
                            }
                            if (!$OnExcludeList) {
?>                           <a onclick="excludelist_add('<?=$UserID?>',true);return false;" href="#" title="Add <?=$Username?> to exclude list"><img src="static/common/symbols/watchedgreen.svg" alt="view" /></a><?php
                            }
?>                          </div>
                              <a onclick="remove_records('<?=$UserID?>');return false;" href="#" title="Remove all speed records belonging to <?=$Username?> from watchlist"><img src="static/common/symbols/trash.svg" alt="del records" /></a>
<?php
                            if ($Enabled=='1') { ?>
                                <a href="tools.php?action=ban_speed_cheat&banuser=1&userid=<?=$UserID?>" title="ban this slag for being a scummy cheater"><img src="static/common/symbols/ban.svg" alt="ban" /></a>
<?php                           }  ?>
                        </td>
                        <td class="center">
<?php                           echo format_username($UserID, $Username, $IsDonor, $Warned, $Enabled, $ClassID, false, false);  ?>
                        ...
                        <td class="center"><img src="static/styles/themes/<?= $LoggedUser['StyleName'] ?>/images/seeders.svg" title="up"/> <?=size_span($Uploaded, get_size($Uploaded))?></td>
                        <td class="center"><?=speed_span($UpSpeed, $SiteOptions[KeepSpeed], 'red', get_size($UpSpeed).'/s')?></td>
...
<?php                           if ($_GET['torrentid']!=$TorrentID) {
                        ?>  <a href="?action=speed_records&viewspeed=0&torrentid=<?=$TorrentID?>" title="View records for just this torrent"><img src="static/common/symbols/view.svg" alt="view" /></a> <?php
                            }
                            if ($GroupID && !array_key_exists($TorrentID, $TWatchlist)) {
                       ?>   <a onclick="twatchlist_add('<?=$GroupID?>','<?=$TorrentID?>',true);" href="#" title="Add torrent to watchlist"><img src="static/common/symbols/watched.svg" alt="view" /></a> <?php
                            }  ?>
                        </td>
                        <td class="center">
                        ...
                        <td class="center"><img src="static/styles/themes/<?= $LoggedUser['StyleName'] ?>/images/leechers.svg" title="down"/> <?=size_span($Downloaded, get_size($Downloaded))?></td>
                        <td class="center"><?=speed_span($DownSpeed, $SiteOptions[KeepSpeed], 'purple', get_size($DownSpeed).'/s')?></td>
