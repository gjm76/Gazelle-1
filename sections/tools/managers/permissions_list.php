...
$DB->query("SELECT p.ID,p.Name,p.Level,p.DisplayStaff,p.MaxSigLength,p.MaxAvatarWidth,
                   p.MaxAvatarHeight, p.Color, p.isAutoPromote, p.reqWeeks, p.reqUploaded, p.reqTorrents, p.reqForumPosts, p.reqRatio, p.reqCredits, 
                   p.reqSnatches, COUNT(u.ID)
...
?>
    <table>
        <tr class="colhead">
            ...
            <td width="10%" class="center">Avatar</td>
            <td width="8%"  class="center">Color</td>
            <td width="9%"  class="center">Staff</td>
            <td width="9%"  class="center" title="Set true for all classes which users can be promoted">Promote</td>
            <td width="5%"  class="center" title="Required Weeks to buy Store Promotion">Weeks</td>
            <td width="5%"  class="center" title="Required Upload to buy Store Promotion">Uploaded</td>
            <?php /*<td width="5%"  class="center" title="NULL">Torrents</td>*/ ?>
            <td width="5%"  class="center" title="Required Snatches to buy Store Promotion">Snatches</td>
            <?php /*<td width="5%"  class="center" title="NULL">Posts</td>*/ ?>
            <?php /*<td width="5%"  class="center" title="NULL">Ratio</td>*/ ?>
            <td width="5%"  class="center" title="Required Cubits to buy Store Promotion">Cubits</td>
            <td width="10%" class="center">Users</td>
            <td width="18%" class="center">Actions</td>
        </tr>
<?php 	while (list($ID,$Name,$Level,$DisplayStaff,$MaxSigLength,$MaxAvatarWidth,$MaxAvatarHeight,$Color,$IsAutoPromote, $reqWeeks, $reqUploaded, 
$reqTorrents, $reqForumPosts, $reqRatio, $reqCredits, $reqSnatches, $UserCount)=$DB->next_record()) {
...
?>
        <tr <?=$IsAutoPromote=='0'&&$DisplayStaff!='1'? 'title="AutoPromote must be turned ON to see requirements"' : ''; ?>  >
            ...
            <td class="center"><?=$IsAutoPromote=='1'? $reqUploaded / pow(1024,3) . 'GB': ''; ?></td>
            <?php /*<td class="center"><?=$IsAutoPromote=='1'? $reqTorrents : ''; ?></td>*/ ?>
            <td class="center"><?=$IsAutoPromote=='1'? $reqSnatches : ''; ?></td>
            <?php /*<td class="center"><?=$IsAutoPromote=='1'? $reqForumPosts : ''; ?></td>*/ ?>
             <?php /*<td class="center" <?=$IsAutoPromote=='1'?'title="Ratio for promotion, demotion ratio is this minus 0.1"' : ''; ?> ><?=$IsAutoPromote=='1'? number_format($reqRatio, 2, '.', '') : ''; ?></td>*/ ?>
            <td class="center"><?=$IsAutoPromote=='1'? $reqCredits : ''; ?></td>
