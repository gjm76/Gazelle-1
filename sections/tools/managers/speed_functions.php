...
{
?>
        <div class="head"><?=$Title?> &nbsp;<img src="static/common/symbols/<?=$TitleIcon?>.svg" alt="view" /><span style="float:right;"><a href="#" onclick="$('#<?=$ListType?>').toggle();this.innerHTML=this.innerHTML=='(hide)'?'(view)':'(hide)';">(hide)</a></span>&nbsp;</div>
...
?>
                    ...
                    <tr class="row<?=$row?>">
                            <td class="center">
                                <a href="?action=speed_records&viewspeed=<?=$ViewSpeed?>&userid=<?=$UserID?>" title="View records for just <?=$Username?>"><img src="static/common/symbols/view.svg" alt="view" /></a>
<?php                          if ($ListType=='watchlist' && $Enabled=='1') { ?>
                                &nbsp;<a href="tools.php?action=ban_speed_cheat&banuser=1&userid=<?=$UserID?>" title="ban this slag for being a scummy cheater (speeding)"><img src="static/common/symbols/ban.svg" alt="ban" /></a>
<?php                          }  ?>
                            </td>
                            ...
                            <!--<td class="center">
                                <input type="checkbox" name="keeptorrent" <?php if($KeepTorrents)echo'checked="checked"'?> value="1" title="if checked keep all torrent records this user is on as well" />
                            </td>-->
                            <td class="center">
