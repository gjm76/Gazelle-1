...
if ($_REQUEST['donategb']) {
    //$add_bytes = floor($amount) * DEDUCT_GB_PER_EURO * 1024 * 1024 * 1024; // 1 euro per gb
    $add_bytes = get_donate_deduction($amount);
    $comment .= "upload: + " . get_size($add_bytes);
} else {
    $comment .= "love";
...
if ($_REQUEST['donategb']) {
    $DB->query("SELECT Uploaded FROM users_main WHERE ID='$UserID'");
    list($uploaded_bytes) = $DB->next_record();

    $Summary = sqltime() . ' - ' . "[url=/donate.php?action=my_donations&amp;userid=$UserID]Donated: &euro;$amount.[/url] Upload added: " . get_size($add_bytes);
    ...
    $DB->query("UPDATE users_info as i JOIN users_main as m ON i.UserID=m.ID
                               SET i.AdminComment=CONCAT_WS( '\n', '".db_string($Summary)."', i.AdminComment),
                                   m.Uploaded=(m.Uploaded+'$add_bytes')
                             WHERE m.ID='$UserID'");

    $Summary = get_size($add_bytes) . " has been added to your upload.";
    ...
    send_pm($UserID, 0, db_string("Thank-you for your donation"), db_string("[br]We have received your donation of &euro;$amount [br][br]:thankyou:[br][br]It's thanks to members like you that this site can carry on <3"));
