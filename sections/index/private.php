...
show_header('News','bbcode', 'featured');
?>
<div class="thin">
    <h1>
        <!--<a style="float:left;margin-top:4px" href="feeds.php?feed=feed_news&amp;user=<?=$LoggedUser['ID']?>&amp;auth=<?=$LoggedUser['RSS_Auth']?>&amp;passkey=<?=$LoggedUser['torrent_pass']?>&amp;authkey=<?=$LoggedUser['AuthKey']?>" title="<?=SITE_NAME?> : News" ><img src="<?=STATIC_SERVER?>/common/symbols/rss.svg" alt="RSS feed" /></a>
    <?=SITE_NAME?> <?=((strtolower(substr( SITE_NAME,0,1))===substr( SITE_NAME,0,1))?'news':'News'); ?>--></h1>

<?php  print_latest_forum_topics(); ?>
<?php  print_premiers(isset($_GET['reload']) && check_perms('site_reload_shows')); ?>
...
<?php
    ...
    list($TVMazeID,$TVMazeTitle,$Synopsis,$TVMazeRating,$TVMazePoster,$SetTime,$Url) = $FeaturedShow;
    if (is_number($TVMazeID) && $TVMazeID != 0) { // is found and set
?>
       <div class="head colhead_dark" style="position:relative;"><strong>Featured Show</strong>
<?php if (check_perms('admin_manage_networks')) {?>
       <a href="tools.php?action=featured_show"><strong>[Edit]</strong></a>
<?php } ?>       
       </div>
        <div class="box" style="vertical-align: middle; text-align: center; padding-top: 20px;">
          <a href='<?=$Url?>' style="text-decoration: none;">
            <table class="featured">
             <tr>    
              <td colspan="2" style="border:0px;">
               <img data-tooltip-width="500" src="<?=$TVMazePoster?>" title="<?=$Synopsis?>">
               <span class="featured-bevels"></span>
              </td>
             </tr>
             <tr> 
               <td style="border:0px; position:relative;"><br>
               <span style="font-weight:bold; font-size:12px;"><?=$TVMazeTitle?></span>
               <div style="font-weight:bold; font-size:12px; float: right;"><?=$TVMazeRating?></div>
               <div class="rating-star"></div><br>
               </td>
             </tr>
            </table>
         </a>        
...
/* Removing staff blog posts, as it's not used right now
...
<?php 	}  ?>
        <div class="head colhead_dark">
            ...
            <!--<a style="float:right;margin-top:4px" href="feeds.php?feed=feed_blog&amp;user=<?=$LoggedUser['ID']?>&amp;auth=<?=$LoggedUser['RSS_Auth']?>&amp;passkey=<?=$LoggedUser['torrent_pass']?>&amp;authkey=<?=$LoggedUser['AuthKey']?>" title="<?=SITE_NAME?> : Blog" ><img src="<?=STATIC_SERVER?>/common/symbols/rss.svg" alt="RSS feed" /></a>-->
        </div>
        <div class="box">
...
<?php       if (check_perms('site_view_stats')) { ?>
                <li class="center">
<?php           if (check_perms('site_stats_advanced')) { ?><br>
                    [<a href="stats.php?action=users">Users</a>] &nbsp;
<?php           }   ?>
                    [<a href="stats.php?action=site">Site History</a>]
<?php           if (check_perms('site_stats_advanced')) { ?>
                    &nbsp;[<a href="stats.php?action=torrents">Torrents</a>]<br>
<?php           }   ?>
                </li>
<?php       } ?>

            <br><li>Site Spawned: <?=time_diff(strtotime('Nov 21 2014 15:38'))?></li>
...
// show stats
if (($ShowCount = $Cache->get_value('stats_show_count')) === false) {
    $DB->query("SELECT count( DISTINCT TVMAZE) FROM torrents_group WHERE TVMAZE");
    list($ShowCount) = $DB->next_record();
    $Cache->cache_value('stats_show_count', $ShowCount, 3600*4); // 4h cache
}

?>
                <li>Shows: <?=number_format($ShowCount)?></li>
<?php
// end show stats

// ratings stats
if (($RatingsCount = $Cache->get_value('stats_ratings_count')) === false) {
    $DB->query("SELECT count(ShowID) FROM shows_ratings");
    list($RatingsCount) = $DB->next_record();
    $Cache->cache_value('stats_ratings_count', $RatingsCount, 3600*4); // 4h cache
}

?>
                <li>User Ratings: <?=number_format($RatingsCount)?></li>
<?php
// end ratings stats

// people stats
if (($PeopleCount = $Cache->get_value('stats_people_count')) === false) {
    $DB->query("SELECT count(ID) FROM persons");
    list($PeopleCount) = $DB->next_record();
    $Cache->cache_value('stats_people_count', $PeopleCount, 3600*4); // 4h cache
}

?>
                <li>People: <?=number_format($PeopleCount)?></li>
<?php
// end people stats
...
?>
                ...
                <li>Seeder/Leecher Ratio: <?=$Ratio?></li><br>
            </ul>
                </div>
